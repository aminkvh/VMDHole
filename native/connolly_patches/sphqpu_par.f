...
C failed, in which case the original serial loop runs unchanged.
      INTEGER*1, ALLOCATABLE ::  SPACC(:)
      INTEGER,   ALLOCATABLE ::  SPNACC(:)
      LOGICAL                    SPPAR_OK
      INTEGER                    NACC, IBASE, IALLOC
      INTEGER*8                  MASKSZ
C end of decs ******************
...
C absurd allocation; over the cap we simply run the original serial loop.
      SPPAR_OK = .FALSE.
      IF ((.NOT.LGRID) .AND. (SPNO.GT.0) .AND. (PTNO.GT.0)) THEN
        MASKSZ = INT(SPNO,8) * INT(PTNO,8)
        IF (MASKSZ .LE. 400000000_8) THEN
          ALLOCATE( SPACC(MASKSZ), SPNACC(SPNO), STAT=IALLOC)
          IF (IALLOC.EQ.0) SPPAR_OK = .TRUE.
        ENDIF
      ENDIF
...
C and its a bit involved put into seperate s/r at the bottom of this file
        ...
        IF (SPPAR_OK) SPACC(1:INT(SPNO,8)*INT(PTNO,8)) = 0
...
C accumulation, and the GOTO 99 re-pass, sit under the LGRID branch.) So the
C expensive part - the culling - is embarrassingly parallel; only the WRITE has
C to stay ordered.
C
C Strategy that keeps output BYTE-IDENTICAL: phase 1 culls in parallel and
C records, per sphere, which of its dots survived (SPACC mask) and how many
C (SPNACC). Phase 2 then walks spheres in strict 1..SPNO order and writes the
C survivors, recomputing each dot's coordinates with the identical expressions
C in the identical order. Recomputing is nearly free (three multiply-adds) next
C to the O(SPNO) neighbour scan it replaces, so this costs one cheap serial pass
C rather than doing the culling twice.
      IF ((.NOT.LGRID) .AND. (SPPAR_OK)) THEN
C phase 1: parallel cull.  DCOUNT/CCOUNT/RVEC3/DIST2/ISWAP* are per-iteration
C scratch and must be PRIVATE; everything else is read-only.
!$OMP   PARALLEL DO SCHEDULE(DYNAMIC,64) DEFAULT(SHARED)
!$OMP&    PRIVATE(SCOUNT,DCOUNT,CCOUNT,RVEC3,DIST2,
!$OMP&            ISWAP1,ISWAP2,ISWAP3,NACC,IBASE)
        DO 340 SCOUNT= 1, SPNO
          IBASE = (SCOUNT-1)*PTNO
          NACC = 0
          IF (
     ...
     &                   SPRAD(SCOUNT)*PTXYZ(ISWAP3,DCOUNT)
              DO 360 CCOUNT = 1, MIN(SPNO-SCOUNT,SCOUNT-1)
                DIST2 = (RVEC3(1)-SPXYZ(1,SCOUNT+CCOUNT))**2 +
     &                  (RVEC3(2)-SPXYZ(2,SCOUNT+CCOUNT))**2 +
     &                  (RVEC3(3)-SPXYZ(3,SCOUNT+CCOUNT))**2
                IF (DIST2.LT.SPRAD(SCOUNT+CCOUNT)**2) GOTO 350
                DIST2 = (RVEC3(1)-SPXYZ(1,SCOUNT-CCOUNT))**2 +
     &                  (RVEC3(2)-SPXYZ(2,SCOUNT-CCOUNT))**2 +
     &                  (RVEC3(3)-SPXYZ(3,SCOUNT-CCOUNT))**2
                IF (DIST2.LT.SPRAD(SCOUNT-CCOUNT)**2) GOTO 350
360           CONTINUE
              DO 370 CCOUNT = MIN(SPNO-SCOUNT,SCOUNT-1)+1,
...
340     CONTINUE
!$OMP   END PARALLEL DO
C phase 2: serial, strictly in sphere order - this is what reproduces the
C original file byte for byte.
        DO 380 SCOUNT= 1, SPNO
          IF (SPNACC(SCOUNT).EQ.0) GOTO 380
          IBASE = (SCOUNT-1)*PTNO
          ISWAP1 = MOD(SCOUNT+2,3) + 1
          ISWAP2 = MOD(SCOUNT  ,3) + 1
          ISWAP3 = MOD(SCOUNT+1,3) + 1
          DO 390 DCOUNT= 1, PTNO
            IF (SPACC(IBASE+DCOUNT).EQ.0) GOTO 390
            RVEC3(1) = SPXYZ(1,SCOUNT) +
     &                 SPRAD(SCOUNT)*PTXYZ(ISWAP1,DCOUNT)
            RVEC3(2) = SPXYZ(2,SCOUNT) +
     &                 SPRAD(SCOUNT)*PTXYZ(ISWAP2,DCOUNT)
...
55555 CONTINUE
      IF (ALLOCATED(SPACC))  DEALLOCATE(SPACC)
      IF (ALLOCATED(SPNACC)) DEALLOCATE(SPNACC)
      RETURN
      END
