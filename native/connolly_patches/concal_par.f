...
C prepass calls CONCAL from inside an OpenMP region, so printing there
C puts the notice at a scheduling-dependent position in the .out.
C
C The prepass sets CN_REC (threadprivate) before each call; CONCAL then
C stores the notice's radius in CN_VAL and raises CN_FLAG rather than
C writing, and holcal_par replays the recorded notices serially in
C ascending plane order. On every other path CN_REC is false and this
C file behaves exactly as upstream, at every SHORTO level.
C
C The values are recorded, not recomputed: the notice's radius comes from
C a HOLEEN call inside CONCAL and is not equal to holcal's own STRRAD.
C Every other line is verbatim upstream.
C ===================================================================
