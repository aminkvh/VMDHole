...
C
C ONLY loop 20 is parallel. Loop 21 reads RESSTN(1,ACOUNT-1) and WRITEs the
C residue-break lines from inside the loop body, so it is ordered by
C construction and is left serial.
C
C Safe because HOLEEN (holeen_par.f) is already thread-safe for the parallel
C CONNOLLY path - its cutoff cache is THREADPRIVATE and its result is
C cache-invariant. RSTORE is written at the loop index only, and the row is
C WRITEn from the outer SCOUNT body, outside the region, so output order is
C unchanged. Byte-identity is asserted by tests/test_h2dmap_parity.sh.
C ********************************************************************
C *                                                                  *
...
C$OMP&PRIVATE(ANGLE,DIRECT,DTOUCH,CCOUNT,NEWCEN,NEWENG)
C$OMP&PRIVATE(IAT1,IAT2,IAT3,DAT2,DAT3)
        DO 20 ACOUNT = 1, ADIV
