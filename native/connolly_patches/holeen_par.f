...
C cache). Safe because HOLEEN's result is cache-invariant (proven: plain vs
C CONN centreline identical despite different cache histories), so
C per-thread caches give identical output. All other locals are now
C automatic (per-call, per-thread) - thread-safe.
C ********************************************************************
C *                                                                  *
...
C PERF FORK: only the cache persists across calls, and it is per-thread.
      SAVE			CUTCENTRE, CUTDIST, CUTSTORE
!$OMP THREADPRIVATE(CUTCENTRE, CUTDIST, CUTSTORE)
...
C exactly (HOLEEN's result is subtly sensitive to cache-rebuild timing).
      ENTRY HRESET
      CUTSTORE(0) = 0
      RETURN
      END
