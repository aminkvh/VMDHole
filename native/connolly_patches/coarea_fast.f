...
C Unmodified original is coarea.f. Untouched: unlike the HCAPEN/CAPSULE
C case, this index amortizes WITHIN a single call (build once, query many
C squares) rather than needing to survive across calls, so there is no
C per-call rebuild tax to pay - profiling showed coarea_ at ~41% of a CONN
C run's time (gprof, 512 calls, ~7.6ms/call average).
C
C Correctness: the black/grey/white classification is an EXISTENTIAL OR
C over circles ("does ANY active circle satisfy the test") - the black
C early-exit (GOTO 30/71) adds a FIXED SIZE**2 regardless of which circle
C triggered it, and the grey flag is a pure OR, so reordering or
C subsetting which circles are visited cannot change the result, PROVIDED
C the subset never omits a circle that could satisfy the (wider) grey
C test. Any circle within QMARGIN = GMAXR + 0.5*ROOT2*SIZE(1) of a square
C centre is captured (GMAXR = the true max active SCOCIRCRAD this call,
C computed once - an exact bound, not a heuristic, so no retry/fallback
C is needed here, unlike the HCAPEN cutoff-list case). SIZE(1) is the
C coarsest (largest) square size used anywhere in this call, so a single
C fixed QMARGIN safely covers every finer refinement cycle too.
      INTEGER			QCELLMAX
      PARAMETER (		QCELLMAX = 200)
      DOUBLE PRECISION		QCELL
      PARAMETER (		QCELL = 3.0D0)
      INTEGER			QNE, QNN, QCIDX
      INTEGER			QSTART(0:QCELLMAX*QCELLMAX)
      INTEGER			QCURSOR(0:QCELLMAX*QCELLMAX)
      INTEGER			QBUCKET(SCOMAX)
      DOUBLE PRECISION		GMAXR, QMARGIN, QREACH
      INTEGER			QIE, QIN, QIELO, QIEHI, QINLO, QINHI
      INTEGER			QPOS, QNCAND, QCC
      INTEGER			QCAND(SCOMAX)
      LOGICAL			QGRIDOK
...
C --- PERF FORK: build the 2D circle-position grid once, from this call's
C finalized SCOENC/SCOCIRCRAD/SCOTDO (unchanged for the remainder of this
C call). QMARGIN = GMAXR + half-diagonal at the coarsest size - an exact
C bound (see correctness note above), safe for every square query below
C AND every later refinement cycle (which only uses smaller SIZE values).
      GMAXR = -1D10
      DO SCOUNT = 1, SCONUM
        IF (SCOTDO(SCOUNT).AND.(SCOCIRCRAD(SCOUNT).LT.ENDRAD)) THEN
          IF (SCOCIRCRAD(SCOUNT).GT.GMAXR) GMAXR = SCOCIRCRAD(SCOUNT)
        ENDIF
      ENDDO
      QMARGIN = GMAXR + 0.5D0*ROOT2*SIZE(1)
      QNE = INT((EMAX-EMIN)/QCELL) + 2
      QNN = INT((NMAX-NMIN)/QCELL) + 2
      QGRIDOK = (QNE.GT.0).AND.(QNN.GT.0).AND.
     &          (QNE.LE.QCELLMAX).AND.(QNN.LE.QCELLMAX)
      IF (QGRIDOK) THEN
        DO QCIDX = 0, QNE*QNN
          QSTART(QCIDX) = 0
        ENDDO
        DO SCOUNT = 1, SCONUM
          IF (SCOTDO(SCOUNT).AND.(SCOCIRCRAD(SCOUNT).LT.ENDRAD)) THEN
            QIE = INT((SCOENC(1,SCOUNT)-EMIN)/QCELL)
            QIN = INT((SCOENC(2,SCOUNT)-NMIN)/QCELL)
            QCIDX = QIE + QIN*QNE
            QSTART(QCIDX+1) = QSTART(QCIDX+1) + 1
          ENDIF
        ENDDO
        DO QCIDX = 1, QNE*QNN
          QSTART(QCIDX) = QSTART(QCIDX) + QSTART(QCIDX-1)
        ENDDO
        DO QCIDX = 0, QNE*QNN-1
          QCURSOR(QCIDX) = QSTART(QCIDX)
        ENDDO
        DO SCOUNT = 1, SCONUM
          IF (SCOTDO(SCOUNT).AND.(SCOCIRCRAD(SCOUNT).LT.ENDRAD)) THEN
            QIE = INT((SCOENC(1,SCOUNT)-EMIN)/QCELL)
...
C go thru "circles" 1 by 1 - PERF FORK: only circles in nearby grid cells
C (see correctness note above), or all SCONUM if the grid wasn't built
          IF (QGRIDOK) THEN
            QIELO = INT((CENENC(1)-QMARGIN-EMIN)/QCELL)
            QIEHI = INT((CENENC(1)+QMARGIN-EMIN)/QCELL)
            QINLO = INT((CENENC(2)-QMARGIN-NMIN)/QCELL)
            QINHI = INT((CENENC(2)+QMARGIN-NMIN)/QCELL)
            IF (QIELO.LT.0) QIELO = 0
            IF (QINLO.LT.0) QINLO = 0
            IF (QIEHI.GT.QNE-1) QIEHI = QNE-1
            IF (QINHI.GT.QNN-1) QINHI = QNN-1
            QNCAND = 0
            DO QIN = QINLO, QINHI
              DO QIE = QIELO, QIEHI
                QCIDX = QIE + QIN*QNE
                DO QPOS = QSTART(QCIDX)+1, QSTART(QCIDX+1)
                  QNCAND = QNCAND + 1
                  QCAND(QNCAND) = QBUCKET(QPOS)
                ENDDO
              ENDDO
            ENDDO
          DO 40 QCC = 1, QNCAND
            SCOUNT = QCAND(QCC)
            IF (.NOT.(SCOTDO(SCOUNT).AND.
     &                (SCOCIRCRAD(SCOUNT).LT.ENDRAD))) GOTO 40
...
40        CONTINUE
          ELSE
          DO 41 SCOUNT = 1, SCONUM
            IF (.NOT.(SCOTDO(SCOUNT).AND.
     &                (SCOCIRCRAD(SCOUNT).LT.ENDRAD))) GOTO 41
            DIFFE = CENENC(1) - SCOENC(1,SCOUNT)
            DIFFN = CENENC(2) - SCOENC(2,SCOUNT)
            DIST2 = DIFFE*DIFFE + DIFFN*DIFFN
            WORK = SCOCIRCRAD(SCOUNT) - 0.5D0*ROOT2*SIZE(1)
...
C look thru all active circles - PERF FORK: grid-filtered candidates
C (see correctness note above), or all SCONUM if the grid wasn't built
          IF (QGRIDOK) THEN
            QIELO = INT((CENENC(1)-QMARGIN-EMIN)/QCELL)
            QIEHI = INT((CENENC(1)+QMARGIN-EMIN)/QCELL)
          ...
          DO 70 QCC = 1, QNCAND
            SCOUNT = QCAND(QCC)
            IF (.NOT.(SCOTDO(SCOUNT).AND.
     &                (SCOCIRCRAD(SCOUNT).LT.ENDRAD))) GOTO 70
            DIFFE = CENENC(1) - SCOENC(1,SCOUNT)
            DIFFN = CENENC(2) - SCOENC(2,SCOUNT)
            DIST2 = DIFFE*DIFFE + DIFFN*DIFFN
...
70        CONTINUE
          ELSE
          DO 75 SCOUNT = 1, SCONUM
            IF (.NOT.(SCOTDO(SCOUNT).AND.
     &                (SCOCIRCRAD(SCOUNT).LT.ENDRAD))) GOTO 75
            DIFFE = CENENC(1) - SCOENC(1,SCOUNT)
            DIFFN = CENENC(2) - SCOENC(2,SCOUNT)
            DIST2 = DIFFE*DIFFE + DIFFN*DIFFN
            WORK = SCOCIRCRAD(SCOUNT) + 0.5D0*ROOT2*SIZE(IFROM)
            WORK = WORK*WORK
            IF (DIST2.GT.WORK) GOTO 75
            DO 81 ECOUNT = 1, 8
              DIFFE = EDGE(1,ECOUNT) - SCOENC(1,SCOUNT)
              DIFFN = EDGE(2,ECOUNT) - SCOENC(2,SCOUNT)
              DIST2 = DIFFE*DIFFE + DIFFN*DIFFN
