#!/bin/sh
# Build every binary VMDPathFinder uses, in one step. This is the install script:
#   * the VMDPathFinder source patches applied (fast CAPSULE ~40x, fast CONNOLLY area,
#     and PARALLEL CONNOLLY ~6.5x via OpenMP) - byte-identical .sph + all numbers
#     to stock HOLE. See connolly_patches/ (originals backed up to *.vmdpathfinder_orig);
...
# point VMDPathFinder's executable paths (File -> Settings...) at a single directory.
# Your existing HOLE install is NOT modified.
#
# Usage:
#   ./build-vmdpathfinder-optimized.sh [HOLE2_SRC_DIR] [OUTPUT_DIR] [INSTALL_DIR]
...
# 1b. Apply the VMDPathFinder source patches (fast CAPSULE + parallel CONNOLLY) ---
# Idempotent; backs up every file it touches to <file>.vmdpathfinder_orig. Adds
...
command -v python3 >/dev/null 2>&1 || {
  ...
  exit 1; }
echo ">> Applying VMDPathFinder CONNOLLY/CAPSULE patches ..."
...
# VMDPathFinder reads this file (next to the hole binary) to show the per-feature
...
{
  echo "vmdpathfinder_accel_manifest 1"
...
} > "$OUT/vmdpathfinder_accel.manifest"
echo ">> Wrote detection manifest: $OUT/vmdpathfinder_accel.manifest (openmp=$OMP_OK)"
...
# arg) to the exe dir VMDPathFinder points at, e.g.
#   HOLE_INSTALL_DIR="$HOME/hole2/exe" ./build-vmdpathfinder-optimized.sh
...
# green acceleration checks light up (VMDPathFinder reads it next to the hole binary).
...
if [ -n "$INSTALL_DIR" ]; then
  if [ ! -d "$INSTALL_DIR" ]; then
    ...
    cp -f "$OUT/vmdpathfinder_accel.manifest" "$INSTALL_DIR/vmdpathfinder_accel.manifest"
    echo ">> Installed. Restart VMDPathFinder (or press Detect in Settings) to pick it up."
  fi
fi
...
Done. Every binary VMDPathFinder uses is in:
    $OUT
...
Point VMDPathFinder at it in File -> Settings... (set hole / sph_process / sos_triangle
to the files above), or copy them over your existing hole2/exe binaries. VMDPathFinder
