...
The directory was historically called `hole_speedup/`, a name it outgrew when
the tunnel engine and hydration tools moved in.
...
Identical output to upstream. **`sos_triangle` itself is ~9.4× faster at the
plugin's default dot density (15)**, growing to **~75× on ~4×10⁴-triangle surfaces**
(see `../paper/benchmarks/results/sos_scaling.csv` for the full, current density
sweep, measured on 9HNR frame 0 and regenerated by
`paper/benchmarks/reproduce.sh --stage sos-sweep`), because two O(N²) bottlenecks (`neighbour`,
`destroy`) become near-linear. Fitting `t = c + a·N^b` over that sweep, using a
model that separates fixed per-run cost from growth, puts the exponent at
**1.95 (upstream) → 1.02 (this work)** with R² > 0.999, which is the evidence
for that claim, and those exponents shift only in the third digit between runs.
(A plain log–log fit reads 1.93 → 0.78; it charges the ~15 ms fixed cost to the
smallest surfaces and so understates the exponent.) It also
raises the polygon cap so large pores that previously aborted now complete.

Scope note: the figure above is the speedup of the `sos_triangle` step alone.
`hole` and `sph_process` are **also** accelerated in this tree (OpenMP-parallel
CONNOLLY driver, fast Connolly-area and capsule-energy routines, parallel
`sph_process` dot cull; see the Fortran patches section of
[`CHANGES.md`](CHANGES.md)). Measured **whole-pipeline** against genuine stock
HOLE 2.2 built at the same `-O2` (1BL8, seeded, output identical):
**11.8× end-to-end** for a circular run at the default dot density 15
(2 457 → 208 ms), and **6.9×** for CONNOLLY at dot density 8
(1 358 → 197 ms). The circular case is higher only because the surface step is
~98 % of that workload; CONNOLLY spreads the cost across all three stages.

⚠️ CONNOLLY is quoted at density 8 rather than the default 15 because **stock
cannot complete it at 15**: upstream `sos_triangle`'s `MAX_COORD 30000` makes it
emit zero triangles from density 12 upward on this structure, so above that there
is no stock result to compare with. Raw data:
[`../paper/benchmarks/pipeline_end_to_end.csv`](../paper/benchmarks/pipeline_end_to_end.csv).
