...
# The engine is inlined in vmdpathfinder.tcl now (one shipped script + the
...
::VMDPathFinder::Mole::read_atoms $atoms A
...
::VMDPathFinder::Mole::complex_build_dh C M $piv $rad 3.0 1.25 8
lassign [::VMDPathFinder::Mole::cavities C cav 8 5.0] nc nch nvd
...
foreach tn $tunnels {
    ...
    dict set lining $lid [dict replace \
        [::VMDPathFinder::Mole::lining [lindex $tn 2] $piv $np $pres $pbb $restab] \
        restab $restab]
}
set cavlines {}
if {$A(has_names)} {
    ...
    for {set c 0} {$c < [llength $cav]} {incr c} {
        ...
        lassign [::VMDPathFinder::Mole::cavity_residues C $c $pres [llength $restab]] bnd inner
        foreach line [::VMDPathFinder::Mole::format_cavity [expr {[lindex $rank $c] + 1}] \
                          $vol $dep $dlen $bnd $inner $restab] { lappend cavlines $line }
    }
}
::VMDPathFinder::Mole::write_tunnels $tout $A(n) $C(nt) 3.0 1.25 $tunnels \
