...
source [file join $REPO vmdpathfinder vmdpathfinder.tcl]
set ::VMDPathFinder::config_file /tmp/.vmdpathfinder_config_stabtest
...
proc run_mode {label cp_stab cv_stab cp_track cv_exact} {
    ...
    set ::VMDPathFinder::state(stabilize_cpoint) $cp_stab
    set ::VMDPathFinder::state(stabilize_cvect)  $cv_stab
    set ::VMDPathFinder::state(track_cpoint)     $cp_track
    set ::VMDPathFinder::state(cvect_exact)      $cv_exact
    molinfo $molid set frame 0
    if {[catch {::VMDPathFinder::_stab_init $molid} e]} { puts "  $label: _stab_init FAILED: $e"; return }
    set cv0 ""; set cp0 ""
    puts "  --- $label ---"
    for {set f 0} {$f < $NFRAMES} {incr f} {
        molinfo $molid set frame $f
        if {[catch {::VMDPathFinder::frame_axis $molid $f} r]} { puts "   f$f frame_axis ERROR: $r"; continue }
        lassign $r cp cv
