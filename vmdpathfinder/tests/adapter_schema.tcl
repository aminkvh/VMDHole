...
if {[catch {
    ...
    set rows [::VMDPathFinder::_tunnel_atoms_mole $mid 0]
    set fails 0
    ...
    foreach {nm rn want} {CA ALA C  CA CA Ca  N ALA N  OD1 ASP O  FE HEM Fe
        ...
        chk "element: name '$nm' in residue '$rn' -> $want" \
            [expr {[::VMDPathFinder::_element_from_atom_name $nm $rn] eq $want}] \
            "(got [::VMDPathFinder::_element_from_atom_name $nm $rn])"
    }
    ...
    set ::VMDPathFinder::state(selection) "all"
    set hrows [::VMDPathFinder::_tunnel_atoms_mole $hid 0]
    set helem {}
    ...
    set prows [::VMDPathFinder::_tunnel_atoms_mole $hid 0]
