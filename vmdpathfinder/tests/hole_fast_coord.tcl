...
set _sv [expr {[info exists ::VMDPathFinder::state(keep_input_pdb)] ? $::VMDPathFinder::state(keep_input_pdb) : 0}]
...
set ::VMDPathFinder::state(keep_input_pdb) $_sv
set _sv_exe $::VMDPathFinder::state(hole_exec)
set ::VMDPathFinder::state(hole_exec) ""
chk "no binary at all means no packed record" [::VMDPathFinder::_hole_fast_coord_available] 0
set ::VMDPathFinder::state(hole_exec) $_sv_exe

if {![file readable $PDB] || ![file readable $RAD]} {
    puts "  ....  no 1GRM fixture - gate checks only"
    done
}
if {![::VMDPathFinder::_hole_fast_coord_available]} {
    ...
    chk "a HOLE without the patch is refused the packed record" \
        [::VMDPathFinder::_hole_fast_coord_available] 0
    done
}

# --- the real thing: same frame, both input paths, identical .sph ------------
set work [file join [::VMDPathFinder::get_temp_base] "vmdpathfinder_fc_[pid]"]
...
foreach mode {pdb vhb} {
    ...
    if {$mode eq "pdb"} {
        ...
        set pr [::VMDPathFinder::_hole_coord_identity $sel [file join $d _id.pdb]]
        if {[llength $pr] != 2} {
        ...
        ::VMDPathFinder::_write_hole_coord_bin $sel [file join $d input_frame.vhb] $n $idb
    }
    ::VMDPathFinder::write_control_file [file join $d hole.inp] $cn hole_out.sph {0 0 0} {0 0 1}
    catch {exec sh -c "cd [::VMDPathFinder::shell_quote $d] && [::VMDPathFinder::shell_quote $::VMDPathFinder::state(hole_exec)] < hole.inp > hole_out.txt 2>&1"}
    set c ""
