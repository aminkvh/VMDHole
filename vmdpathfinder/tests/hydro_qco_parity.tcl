...
source [file join [file dirname $here] vmdpathfinder.tcl]

namespace eval ::VMDPathFinder {}
# HOLE install location: $VMDPATHFINDER_HOLE_EXE_DIR, else ~/hole2/exe (the install
...
set ::VMDPathFinder::state(sos_triangle_exec) [file join $_exedir sos_triangle]
set ::VMDPathFinder::state(hole_exec) [file join $_exedir hole]
...
proc ref_qco {wres wpos mx my mz ux uy uz cmin cmax penv dcap} {
    ...
    foreach p $wpos {
        ...
        set rr [::VMDPathFinder::envelope_radius $penv $co]
        if {$rr <= 0} { continue }
...
set a [::VMDPathFinder::_hydro_qco_c $wres $wpos $mx $my $mz $ux $uy $uz $cmin $cmax $penv $dcap]
