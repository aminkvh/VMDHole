...
if {\$nfr < 2} {
    ...
    set root \$::VMDPathFinder::tunnel_root
    set orig_frames \$::VMDPathFinder::tunnel_result_frames
    array set orig_results {}
    foreach fr \$orig_frames { set orig_results(\$fr) \$::VMDPathFinder::tunnel_results(\$fr) }
    ...
    ::VMDPathFinder::import_tunnel_results_from_folder \$root
    set match 1
    if {\$::VMDPathFinder::tunnel_result_frames ne \$orig_frames} { set match 0 }
    foreach fr \$orig_frames {
        if {![info exists ::VMDPathFinder::tunnel_results(\$fr)] \
                || \$::VMDPathFinder::tunnel_results(\$fr) ne \$orig_results(\$fr)} { set match 0 }
    }
    if {\$match} { puts \$o "PASS round-trip byte-identical" } else {
        puts \$o "FAIL round-trip mismatch (frames=\$::VMDPathFinder::tunnel_result_frames want=\$orig_frames)"
        incr bad
    }
    ...
    set ::VMDPathFinder::tunnel_result_frames {}
    ::VMDPathFinder::import_tunnel_results_from_folder \$root
    set skip_ok [expr {![info exists ::VMDPathFinder::tunnel_results(\$fr1)] \
        && [lsearch -exact \$::VMDPathFinder::tunnel_result_frames \$fr1] < 0 \
        && [string match "*skipped*\$fr1*" \$::VMDPathFinder::state(status)]}]
    if {\$skip_ok} { puts \$o "PASS missing out.dat skipped and named in status" } else {
        puts \$o "FAIL missing out.dat not handled correctly: \$::VMDPathFinder::state(status)"
        incr bad
    }
    file rename [file join \$fd1 out.dat.bak] [file join \$fd1 out.dat]

    # ---- 3. unified import on a tunnel-only directory ----
    set ::VMDPathFinder::state(import_dir) \$root
    array unset ::VMDPathFinder::tunnel_results; array set ::VMDPathFinder::tunnel_results {}
    set ::VMDPathFinder::tunnel_result_frames {}
    if {[catch {::VMDPathFinder::import_all_results_from_folder} uerr]} {
        puts \$o "FAIL import_all_results_from_folder errored on tunnel-only dir: \$uerr"
        incr bad
    } elseif {[llength \$::VMDPathFinder::tunnel_result_frames] == 0} {
    ...
    set ::VMDPathFinder::tunnel_shown_cid(1) 0
    set ::VMDPathFinder::tunnel_gear_cid(1,color) red
    set ::VMDPathFinder::state(tunnel_selected_cid) 999
    set ::VMDPathFinder::state(tunnel_shown_default) 0
    array unset ::VMDPathFinder::tunnel_results; array set ::VMDPathFinder::tunnel_results {}
    set ::VMDPathFinder::tunnel_result_frames {}
    ::VMDPathFinder::import_tunnel_results_from_folder \$root
    set cleared [expr {![info exists ::VMDPathFinder::tunnel_shown_cid(1)] \
        && ![info exists ::VMDPathFinder::tunnel_gear_cid(1,color)] \
        && \$::VMDPathFinder::state(tunnel_selected_cid) eq "" \
        && \$::VMDPathFinder::state(tunnel_shown_default) == 1}]
    ...
    set ::VMDPathFinder::tunnel_gear_cid(1,color) red
    ::VMDPathFinder::run_tunnel_analysis
    if {[info exists ::VMDPathFinder::tunnel_gear_cid(1,color)]} {
...
file mkdir \$WORK2
set ::VMDPathFinder::state(cpoint) "73.853 26.536 26.594"
set ::VMDPathFinder::state(work_dir) \$WORK2
...
array unset ::VMDPathFinder::tunnel_results; array set ::VMDPathFinder::tunnel_results {}
set ::VMDPathFinder::tunnel_result_frames {}
set ::VMDPathFinder::state(import_dir) \$WORK2
::VMDPathFinder::import_all_results_from_folder
set both [expr {[llength \$::VMDPathFinder::result_frames] > 0 && [llength \$::VMDPathFinder::tunnel_result_frames] > 0}]
if {\$both} { puts \$o "PASS combined HOLE+tunnel import from one common directory" } else {
    puts \$o "FAIL combined import: HOLE frames=\$::VMDPathFinder::result_frames tunnel frames=\$::VMDPathFinder::tunnel_result_frames"
    incr bad
}
