...
This is why VMDPathFinder's tunnel engine reports **8.44 Å** on 1MXT where MOLE 2
reports **12.04 Å** — the one documented deviation from MOLE's own output. The
deviation is pinned by `vmdpathfinder/tests/mole_upstream_deviation.py` so it cannot
...
VMDPathFinder's engine exactly on both structures** — the strongest confirmation
...
- The defect is **not replicated** in VMDPathFinder's engine. Being wrong in the same
  way as the reference is a poor trade; VMDPathFinder reports the tunnel MOLE's own
  filter selects.
