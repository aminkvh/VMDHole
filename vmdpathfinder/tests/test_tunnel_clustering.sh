...
set ::VMDPathFinder::state(tunnel_start) "73.853 26.536 26.594"
set ::VMDPathFinder::state(mole_engine_exec) "$ENGINE"
set ::VMDPathFinder::state(work_dir) "$WORK"
set ::VMDPathFinder::state(frame_spec) "all"
::VMDPathFinder::run_tunnel_analysis
set nfr [llength \$::VMDPathFinder::tunnel_result_frames]
if {\$nfr < 2} {
    puts \$o "FAIL run produced \$nfr frame(s), need >=2: \$::VMDPathFinder::state(status)"
    incr bad
} else {
    set fr0 [lindex \$::VMDPathFinder::tunnel_result_frames 0]
    set fr1 [lindex \$::VMDPathFinder::tunnel_result_frames 1]

    # ---- 2. cross-frame identity built even with display clustering off ----
    if {!\$::VMDPathFinder::state(tunnel_cluster_on)} {
        puts \$o "FAIL default tunnel_cluster_on is not 1 in this session"
        incr bad
    }
    if {[llength \$::VMDPathFinder::tunnel_xclusters] > 0} {
        puts \$o "PASS cross-frame clusters built alongside display clustering (\$::VMDPathFinder::state(status))"
    } else {
    ...
    ::VMDPathFinder::_tunnel_ensure_clusters_for \$fr0
    if {[info exists ::VMDPathFinder::tunnel_clusters(\$fr0)]} {
    ...
    set cand0 [::VMDPathFinder::_tunnel_candidates \$fr0]
    set raw0 [llength \$::VMDPathFinder::tunnel_results(\$fr0)]
    ...
    set ::VMDPathFinder::state(tunnel_cluster_on) 0
    set candoff [::VMDPathFinder::_tunnel_candidates \$fr0]
    ...
    animate goto \$fr0
    set ::VMDPathFinder::state(tunnel_selected_id) [lindex \$cand0 0]
    set xcid [::VMDPathFinder::_tunnel_selected_cluster]
    set rk1 [::VMDPathFinder::_tunnel_rank_in_frame \$fr1]
    if {\$xcid ne "" && \$rk1 ne ""} {
    ...
    set xc_before \$::VMDPathFinder::tunnel_xclusters
    ::VMDPathFinder::_tunnel_cluster_toggle_clicked
    set xc_after \$::VMDPathFinder::tunnel_xclusters
    ...
    set ::VMDPathFinder::state(tunnel_cluster_on) 1
    ::VMDPathFinder::run_tunnel_analysis
    if {[info exists ::VMDPathFinder::tunnel_clusters(\$fr0)] && [llength \$::VMDPathFinder::tunnel_xclusters] > 0} {
        puts \$o "PASS display clustering ON still builds cross-frame identity alongside it"
    } else {
        puts \$o "FAIL cluster_on=1 rerun: tunnel_clusters exists=[info exists ::VMDPathFinder::tunnel_clusters(\$fr0)] xclusters=[llength \$::VMDPathFinder::tunnel_xclusters]"
        incr bad
    }
    ...
    set _mdsave \$::VMDPathFinder::state(tunnel_cluster_maxdev)
    set ::VMDPathFinder::state(tunnel_cluster_maxdev) 12
    ::VMDPathFinder::_tunnel_apply_maxdev _D _HD [list {{0 0 0}} {{1 0 0}}] 2 3.0
    ...
    set _kern [::VMDPathFinder::tunnel_cluster \$_tuns 3.0]
    rename ::VMDPathFinder::sos_triangle_has_feature ::VMDPathFinder::_tc_real_feat
    proc ::VMDPathFinder::sos_triangle_has_feature {feat} {
        if {\$feat eq "tunnelcluster"} { return 0 }
        return [::VMDPathFinder::_tc_real_feat \$feat]
    }
    set _ref [::VMDPathFinder::tunnel_cluster \$_tuns 3.0]
    rename ::VMDPathFinder::sos_triangle_has_feature {}
    rename ::VMDPathFinder::_tc_real_feat ::VMDPathFinder::sos_triangle_has_feature
    ...
    set ::VMDPathFinder::state(tunnel_cluster_maxdev) \$_mdsave

    # ---- the tunnel kernels answer through the resident mesher ----
    if {[::VMDPathFinder::sos_triangle_has_feature tunnelserve]} {
        set _mids [list [list {0 0 0} {0 0 1} {0 0 2}] [list {1 0 0} {1 0 1} {1 0 2}] [list {9 0 0} {9 0 1} {9 0 2}]]
        set ::VMDPathFinder::_tunnel_serve -1
        set _srv [::VMDPathFinder::_tunnel_cluster_c \$_mids 3.0 12]
        set _via \$::VMDPathFinder::_tunnel_serve
        set ::VMDPathFinder::_tunnel_serve 0
        set _exe [::VMDPathFinder::_tunnel_cluster_c \$_mids 3.0 12]
        set ::VMDPathFinder::_tunnel_serve -1
    ...
    if {[::VMDPathFinder::sos_triangle_has_feature tunnelclusterdist]} {
        ...
        array set _rd {}
        set _cc [::VMDPathFinder::_tunnel_cluster_c [list \$A \$B] 3.0 0 _rd]
        set want [::VMDPathFinder::_tunnel_pair_distance \$A \$B]
        ...
        set _ch [::VMDPathFinder::_tunnel_cluster_c \$chain 3.0 0]
        ...
        foreach c \$::VMDPathFinder::tunnel_xclusters {
            if {[llength \$c] < 2} { continue }
            set first [lindex [lsort -integer -index 0 \$c] 0]
            set cref [::VMDPathFinder::_tunnel_mid_points \
                [::VMDPathFinder::_tunnel_tuple_for [lindex \$first 0] [lindex \$first 1]]]
            foreach m \$c {
                if {\$m eq \$first} { continue }
                if {![info exists ::VMDPathFinder::tunnel_xrepr_dist(\$m)]} { continue }
                incr nchk
                set ref [::VMDPathFinder::_tunnel_pair_distance \$cref \
                    [::VMDPathFinder::_tunnel_mid_points \
                        [::VMDPathFinder::_tunnel_tuple_for [lindex \$m 0] [lindex \$m 1]]]]
                set dd [expr {abs(\$ref - \$::VMDPathFinder::tunnel_xrepr_dist(\$m))}]
