#!/bin/sh
# VMDPathFinder regression suite.
#
...
#   test_release_integrity  builds packaging/vmdpathfinder.zip fresh from git ls-files
...
#   test_mole_engine_ext    the C MOLE engine's additions beyond MOLE's own
#                           output: cavity VP spheres, repeatable --origin=
#                           and --exit=, --vdw= overrides. Catches: a VP
#                           sphere that is not the tunnel's own quantity, a
#                           second origin or exit silently dropped, a bad
#                           --vdw entry running on the built-in radius.
...
#   test_inline_current     the HOLE engine inlined in vmdpathfinder.tcl is GENERATED
#                           from vmdpathfinder/hole_tcl/ - re-inline into a copy and require
...
#         VMDPATHFINDER_RELEASE=1 ./run_tests.sh
...
RELEASE="${VMDPATHFINDER_RELEASE:-0}"
# Hermetic config: every group that drives the real plugin would otherwise
# read the USER'S ~/.vmdpathfinder_config (stale engine paths from another checkout
...
if [ -z "${VMDPATHFINDER_CONFIG_FILE:-}" ]; then
    VMDPATHFINDER_CONFIG_FILE=$(mktemp); export VMDPATHFINDER_CONFIG_FILE
    trap 'rm -f "$VMDPATHFINDER_CONFIG_FILE"' EXIT INT TERM
fi

TEST_GROUPS="test_headless_smoke test_accel_parity test_hydro_qco_parity test_hole_tcl_fallback test_hole_tcl_pore_methods test_hole_tcl_fallback_e2e test_hole_fast_coord test_capsule_incomplete test_ellipse_parity test_h2dmap_parity test_release_integrity test_tcl_pitfalls test_tunnel_separation test_tunnel_clustering test_tunnel_import test_mole_tcl_port test_mole_engine_ext test_cavity_tracking test_hcapen_cache test_inline_current test_adapter_schema test_gui_reachable test_mean_axis test_nm_engine test_conn_lobes_engine test_mesh_csg_engine"
...
for t in $TEST_GROUPS; do
    ...
    # VMDPATHFINDER_RELEASE=1 that is a FAILURE, so one benign internal skip could
...
if [ -n "$skipped" ]; then
    ...
    if [ "$RELEASE" != "0" ]; then
        echo "VMDPATHFINDER_RELEASE set - a skipped group is a FAILURE for a release build."
        fails=$((fails+1))
    else
        echo "  ^ these checked nothing. Re-run with VMDPATHFINDER_RELEASE=1 before tagging."
    fi
fi
