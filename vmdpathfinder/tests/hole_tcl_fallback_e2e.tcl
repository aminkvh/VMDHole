...
# Honour VMDPATHFINDER_HOLE_EXE_DIR like the rest of the suite; ~/hole2/exe stays the
...
set _exedir [expr {[info exists ::env(VMDPATHFINDER_HOLE_EXE_DIR)] && $::env(VMDPATHFINDER_HOLE_EXE_DIR) ne ""
                   ? $::env(VMDPATHFINDER_HOLE_EXE_DIR) : [file join $::env(HOME) hole2 exe]}]
...
if {[::VMDPathFinder::_hole_tcl_exe] eq ""} {
    puts "SKIP: hole_tcl_fallback_e2e - no tclsh for the fallback"
    puts "E2E-RESULT pass=$pass fail=$fail"
    quit
}

set work [file join [::VMDPathFinder::get_temp_base] "vmdpathfinder_e2e_[pid]"]
file mkdir $work
...
if {[catch {::VMDPathFinder::validate_inputs} verr]} {
    chk "validate_inputs routes to the fallback" "error: $verr" "no error"
} else {
    chk "validate_inputs routes to the fallback" $::VMDPathFinder::state(hole_tcl_fallback) 1
}
...
if {[catch {::VMDPathFinder::run_analysis} rerr]} {
    ...
    if {[llength $::VMDPathFinder::result_frames]} {
        ...
        set rd [file join $::VMDPathFinder::state(last_root_dir) [format "frame_%05d" $f]]
...
set work2 [file join [::VMDPathFinder::get_temp_base] "vmdpathfinder_e2e_surf_[pid]"]
file mkdir $work2
array set ::VMDPathFinder::state [list sph_process_exec {} sos_triangle_exec {} \
...
if {[catch {::VMDPathFinder::validate_inputs} verr]} {
    ...
    chk "sph_process routes to the fallback"  $::VMDPathFinder::state(sph_process_exec)  {}
    chk "sos_triangle routes to the fallback" $::VMDPathFinder::state(sos_triangle_exec) {}
    ...
    if {[catch {::VMDPathFinder::run_analysis} rerr]} {
        ...
        set f2 [lindex $::VMDPathFinder::result_frames 0]
        set sph [dict get $::VMDPathFinder::results $f2 sph_file]
        set sos [file join $work2 e2e.sos]
        set plot [file join $work2 e2e.plot]
        if {![file exists $sph]} {
            chk "the run produced a .sph" 0 1
        } elseif {[catch {::VMDPathFinder::run_sph_process $sph $sos 1 4} serr]} {
            chk "run_sph_process works with no binary" "error: $serr" "no error"
        } elseif {[catch {::VMDPathFinder::run_sos_triangle $sos $plot} terr]} {
            chk "run_sos_triangle works with no binary" "error: $terr" "no error"
        } else {
            chk "the surface has real geometry" \
                [::VMDPathFinder::surface_has_geometry $plot] 1
            ...
            set mh [open $msh w]
            puts $mh [::VMDPathFinder::_surface_mesh_script 4 $sph \
            ...
            if {[catch {exec sh $msh} merr]} {
                ...
                chk "the tunnel mesh script produces geometry" \
                    [::VMDPathFinder::surface_has_geometry [file join $work2 m.plot]] 1
            }
...
foreach {meth dm} {capsule triangulated connolly triangulated} {
    set w3 [file join [::VMDPathFinder::get_temp_base] "vmdpathfinder_e2e_${meth}_[pid]"]
    file mkdir $w3
    array set ::VMDPathFinder::state [list frame_spec 0 hole_exec {} sph_process_exec {} \
    ...
    if {[catch {::VMDPathFinder::validate_inputs} verr]} {
    ...
    } elseif {[catch {::VMDPathFinder::run_analysis} rerr]} {
        chk "$meth: run_analysis completes" "error: $rerr" "no error"
    } else {
        set f3 [lindex $::VMDPathFinder::result_frames 0]
        set p3 [dict get $::VMDPathFinder::results $f3 profile]
        ...
        set s3 [dict get $::VMDPathFinder::results $f3 sph_file]
        chk "$meth: wrote a .sph" [expr {[file exists $s3] && [file size $s3] > 0}] 1
        if {$dm ne "none"} {
            if {[catch {::VMDPathFinder::create_plot_asset [file dirname $s3] $s3 $dm \
                            $mid $f3} a3]} {
                chk "$meth: builds a surface" "error: $a3" "no error"
            } else {
                chk "$meth: builds real geometry" \
                    [expr {[dict get $a3 kind] eq "vmd_plot" &&
                           [::VMDPathFinder::surface_has_geometry [dict get $a3 path]]}] 1
            }
        }
    }
