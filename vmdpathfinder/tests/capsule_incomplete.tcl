...
# vmdpathfinder_status.dat records the ENGINE's exit status, so it cannot see this:
...
set work [file join [::VMDPathFinder::get_temp_base] "vmdpathfinder_incomplete_[pid]"]
file mkdir $work
...
proc run_method {meth mid work RAD REF} {
    ...
    if {[catch {::VMDPathFinder::run_analysis} err]} { return [list error $err] }
    if {![llength $::VMDPathFinder::result_frames]} { return [list norows ""] }
    set f [lindex $::VMDPathFinder::result_frames 0]
    return [list ok [dict get $::VMDPathFinder::results $f run_dir]]
}
...
if {$st eq "ok"} {
    ...
    set side [file join $cap_dir vmdpathfinder_incomplete.dat]
    chk "the job wrote an incomplete sidecar" [file exists $side] 1

    set raw [::VMDPathFinder::_hole_frame_incomplete $cap_dir]
    note "sidecar = \"$raw\""
    ...
    chk "...naming the +VE direction" [::VMDPathFinder::_incomplete_dirs_label $raw] "+"
    ...
    lassign [::VMDPathFinder::_sph_side_counts $cap_dir] np nn
    ...
    chk "the gate drops a warning both sides contradict" \
        [::VMDPathFinder::_incomplete_real_dirs $cap_dir $raw] ""
    set surfaced 0
    foreach e $::VMDPathFinder::msg_log {
...
if {$st2 eq "ok"} {
    set raw2 [::VMDPathFinder::_hole_frame_incomplete $sph_dir]
...
file mkdir $axdir
set afh [open [file join $axdir vmdpathfinder_frame_axis.dat] w]
...
chk "a one-sided profile is flagged, and names the starved side" \
    [::VMDPathFinder::_profile_one_sided $axdir $p_starved] "-"
# The mirror image.
chk "...and the other side too" \
    [::VMDPathFinder::_profile_one_sided $axdir [dict create xvalues {-10.0 -2.0 3.0 4.5}]] "+"
...
chk "a balanced profile is NOT flagged" \
    [::VMDPathFinder::_profile_one_sided $axdir [dict create xvalues {-20.0 0.0 10.0 18.0}]] ""
# Lopsided but not starved: 3 A on the short side clears the 2 A floor.
chk "merely lopsided is NOT flagged" \
    [::VMDPathFinder::_profile_one_sided $axdir [dict create xvalues {1.0 6.0 12.0 18.0}]] ""
# No axis file (an imported or older run) - no evidence, so no claim.
chk "no axis file yields no claim" \
    [::VMDPathFinder::_profile_one_sided [file join $work nosuch] $p_starved] ""

# 4. The label helper reads BOTH signs out of HOLE's own wording.
chk "both directions collapse to one label" \
    [::VMDPathFinder::_incomplete_dirs_label \
        "MAY BE INCOMPLETE IN +VE DIRECTION MAY BE INCOMPLETE IN -VE DIRECTION"] "+ and -"
chk "-VE alone" [::VMDPathFinder::_incomplete_dirs_label "MAY BE INCOMPLETE IN -VE DIRECTION"] "-"
chk "no warning yields no label" [::VMDPathFinder::_incomplete_dirs_label ""] ""
