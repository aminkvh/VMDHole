...
set rc [::VMDPathFinder::Mole::dt_build_quantised M $points $npt]
...
if {[llength $argv] > 2} {
    ::VMDPathFinder::Mole::read_atoms [lindex $argv 2] A
    set piv [::VMDPathFinder::Mole::pivot_points A]
    set np [expr {[llength $piv] / 3}]
    if {$np > $npt} { set np $npt; set piv [lrange $piv 0 [expr {3 * $npt - 1}]] }
    set got [::VMDPathFinder::Mole::quantise_points $piv $np]
    set nq 0
    ...
    set rc2 [::VMDPathFinder::Mole::dt_build B $piv $np]
