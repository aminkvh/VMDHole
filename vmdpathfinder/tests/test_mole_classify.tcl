...
set nbb 0; set naa 0
foreach n $BB { if {[::VMDPathFinder::Mole::is_backbone_name $n]} { incr nbb } }
report "all [llength $BB] backbone names classify backbone" \
       [expr {$nbb == [llength $BB]}] "(got $nbb)"
foreach n $NOTBB { if {[::VMDPathFinder::Mole::is_backbone_name $n]} { lappend badbb $n } }
report "side chains, metals and waters are not backbone" \
       [expr {![info exists badbb]}] "([expr {[info exists badbb] ? $badbb : {}}])"
foreach r $AA { if {[::VMDPathFinder::Mole::is_amino_name $r]} { incr naa } }
report "all 20 amino names classify amino" [expr {$naa == 20}] "(got $naa)"
foreach r $NOTAA { if {[::VMDPathFinder::Mole::is_amino_name $r]} { lappend badaa $r } }
...
report "classification is case-insensitive" \
       [expr {[::VMDPathFinder::Mole::is_backbone_name "ca"] &&
              [::VMDPathFinder::Mole::is_backbone_name "o5'"] &&
              [::VMDPathFinder::Mole::is_amino_name "his"] &&
              ![::VMDPathFinder::Mole::is_amino_name "hem"]}]
...
report "H is a backbone name (MOLE's set, not a typo)" \
       [::VMDPathFinder::Mole::is_backbone_name H]
...
if {[llength $argv] > 0 && [file readable [lindex $argv 0]]} {
    ...
    foreach line [split [read $fh] "\n"] {
        ...
        set mine [expr {$kind eq "BB" ? [::VMDPathFinder::Mole::is_backbone_name $name]
                                      : [::VMDPathFinder::Mole::is_amino_name $name]}]
