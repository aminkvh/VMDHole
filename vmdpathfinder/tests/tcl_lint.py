...
user reaches it.
  ...
  dead-callback    -command / bind naming a ::VMDPathFinder:: proc that does not
...
def main():
    ...
    # --- dead-callback -------------------------------------------------------
    for i, l in enumerate(lines):
        for m in re.finditer(r'-command\s+(?:\{\s*)?(::VMDPathFinder::[A-Za-z_][\w:]*)', l):
            if m.group(1) not in procs:
                findings.append(("dead-callback", i + 1,
                                 f"-command {m.group(1)} - no such proc"))
        for m in re.finditer(r'bind\s+\S+\s+<[^>]+>\s+(::VMDPathFinder::[A-Za-z_][\w:]*)', l):
            if m.group(1) not in procs:
                findings.append(("dead-callback", i + 1,
