...
# Honour VMDPATHFINDER_HOLE_EXE_DIR like the rest of the suite (~/hole2/exe stays
# the install default; an explicit EXE= wins over both).
if [ -z "${EXE:-}" ] && [ -n "${VMDPATHFINDER_HOLE_EXE_DIR:-}" ]; then EXE="$VMDPATHFINDER_HOLE_EXE_DIR"; fi
