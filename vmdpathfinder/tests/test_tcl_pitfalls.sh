...
foreach p {kd ww kr charge polarity lipophilicity esp gz dens pfdens zzz-unknown} {
    if {[catch {::VMDPathFinder::property_meta $p} r]} { puts $out "FAIL property_meta $p: $r"; incr bad }
}
foreach k {kd ww kr charge polarity lipophilicity esp gz dens pfdens zzz-unknown} {
    if {[catch {::VMDPathFinder::scheme_display_label $k}]} { puts $out "FAIL scheme_display_label $k"; incr bad }
}
puts $out "BAD=$bad"
close $out
quit
