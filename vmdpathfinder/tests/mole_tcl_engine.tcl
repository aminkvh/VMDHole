...
#
# The engine used to ship as eight mole_*.tcl modules beside vmdpathfinder.tcl, and
# every test here sourced whichever subset it needed. It is now INLINED into
# vmdpathfinder.tcl (the plugin ships as one script plus the compiled binaries), so
# those files no longer exist. This file replaces all of those `source` lines.
#
# vmdpathfinder.tcl itself cannot simply be sourced here: it is ~1.5 MB of VMD/Tk
...
# region - which is self-contained ::VMDPathFinder::Mole code, no VMD dependency -
# and source that.
#
...
proc ::_load_inlined_mole_engine {} {
    ...
    if {[llength $b] != 1 || [llength $e] != 1} {
        error "mole_tcl_engine: expected exactly 1 BEGIN and 1 END sentinel in\
               vmdpathfinder.tcl, found [llength $b] BEGIN and [llength $e] END -\
    ...
    set tmp [file join $tmpdir "vmdpathfinder_mole_engine_[pid].tcl"]
    set oh [open $tmp w]
    puts $oh $body
    close $oh
    ...
    if {![llength [info procs ::VMDPathFinder::Mole::find_tunnels]]} {
        error "mole_tcl_engine: sourced the inlined region but\
               ::VMDPathFinder::Mole::find_tunnels is still undefined -\
