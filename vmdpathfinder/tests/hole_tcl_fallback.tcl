...
# they are lifted straight out of vmdpathfinder.tcl rather than reimplemented here -
...
set SRC  [file join $HERE .. vmdpathfinder.tcl]
# Same rule hydro_qco_parity.tcl uses: honour VMDPATHFINDER_HOLE_EXE_DIR, fall back to
...
# ---- lift the fallback's plugin-side procs out of vmdpathfinder.tcl ----------------
...
foreach ln [split $text "\n"] {
    if {$buf eq "" && ![string match "proc ::VMDPathFinder::_hole_tcl_*" $ln]} continue
    append buf $ln "\n"
    if {[info complete $buf]} { append lifted $buf; set buf "" }
}
if {[llength [regexp -all -inline {proc ::VMDPathFinder::_hole_tcl_\w+} $lifted]] < 5} {
    puts "  FAIL hole_tcl_fallback: lifted only [regexp -all {proc ::VMDPathFinder::_hole_tcl_} $lifted]\
          fallback procs from vmdpathfinder.tcl"
    exit 1
}
eval $lifted

set TMP [file join [expr {[info exists ::env(TMPDIR)] ? $::env(TMPDIR) : "/tmp"}] \
             "vmdpathfinder_fbtest_[pid]"]
file mkdir $TMP
proc ::VMDPathFinder::get_temp_base {} { return $::TMP }
proc ::vmdcon {args} {}
set ::VMDPathFinder::plugin_dir [file normalize [file join $HERE ..]]

# ---- 1. the engine extracts, the driver parses, the script self-checks -------
set script [::VMDPathFinder::_hole_tcl_script]
if {$script eq ""} {
...
if {![file readable $PDB] || ![file readable $RAD]} {
    ...
    lassign [::VMDPathFinder::_hole_tcl_args_from_inp $inp] st args
    if {$st ne "ok"} {
        ...
        set tclsh [::VMDPathFinder::_hole_tcl_exe]
        cd $TMP
...
set hdr_driver ""
if {[regexp {puts \$fh "(coord\\tradius[^"]*)"} [::VMDPathFinder::_hole_tcl_driver] -> h]} {
    set hdr_driver $h
}
if {$hdr_plugin eq "" || $hdr_driver eq ""} {
    no "failed-frame header is pre-written" "could not find the header in vmdpathfinder.tcl"
} else {
    ...
    catch {exec {*}[::VMDPathFinder::_hole_tcl_exe] $script -pdb empty.pdb \
    ...
    catch {exec {*}[::VMDPathFinder::_hole_tcl_exe] $script -pdb [file join $TMP in.pdb] \
...
foreach {name card} {unknown "2dmaps 1" bad-conn "conn wide"
    ...
    puts $h "stop"
    close $h
    lassign [::VMDPathFinder::_hole_tcl_args_from_inp $p] st why
...
foreach {card want} {conn {-method connolly}
    ...
    puts $h "stop"
    close $h
    lassign [::VMDPathFinder::_hole_tcl_args_from_inp $p] st args
    set got {}
