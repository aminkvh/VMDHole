#!/usr/bin/env python3
"""Re-inline vmdpathfinder/hole_tcl/ into vmdpathfinder.tcl's sentinel-delimited engine region.

The plugin ships as ONE script, so the engine lives inside vmdpathfinder.tcl rather
than beside it. Editing anything under vmdpathfinder/hole_tcl/ therefore has no effect until
this runs:

    python3 vmdpathfinder/hole_tcl/sync_into_plugin.py vmdpathfinder/vmdpathfinder.tcl
...
if __name__ == "__main__":
    ...
    main(sys.argv[1] if len(sys.argv) > 1 else "vmdpathfinder/vmdpathfinder.tcl")
