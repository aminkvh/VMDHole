...
#  binary, but with VMDPathFinder in mind: the entry points, the .sph output and the
#  radius profile all use the shapes vmdpathfinder.tcl already parses, so a working
...
#  its terms apply - see vmdpathfinder/NOTICE.md.
#
# ------------------------------------------------------------------------------
#
...
#    * the .sph writer, BYTE-IDENTICAL to the reference-build hole (see vmdpathfinder/hole_tcl/README.md)'s own sphpdb
...
#      reproducing command and vmdpathfinder/hole_tcl/tests/sph_addend_test.sh for the
...
#      centre's own precision, not a porting bug (see vmdpathfinder/hole_tcl/connolly.tcl's
...
#  file implements, despite looking like it should - see vmdpathfinder/hole_tcl/refine.tcl:
...
#  vmdpathfinder/hole_tcl/tests/profile_vs_reference.sh - but this has only been checked
...
proc hole::rng::srand {s} {
    ...
    # stays a literal SRAND reproduction because vmdpathfinder/hole_tcl/tests/rng_stream_test.sh
...
proc hole::rng::rand {} {
    ...
    # vmdpathfinder/hole_tcl/tests/rng_stream_test.sh's SRAND(1)+RAND(0)x50 reference never
...
# vmdpathfinder/hole_tcl/tests/fixtures/1BL8_protein_only.pdb's own REMARK for the worked
...
proc hole::radius_for {rules atname resname} {
    ...
    if {[llength $rules] > 0} {
        ...
        # (K+ with simple.rad) and vmdpathfinder/hole_tcl/tests/fixtures/
...
#  (see vmdpathfinder/hole_tcl/README.md's HOLCAL trace section) but it removes a genuine,
...
proc hole::anneal_slice {cx cy cz vx vy vz n xs ys zs rs mcstep mclen mcktin} {
    ...
    # vmdpathfinder/hole_tcl/refine.tcl for the full account. So LOWCEN/LOWENG from the
...
#  - Verified against the reference-build hole (see vmdpathfinder/hole_tcl/README.md) with an actual `sphpdb` card (1GRM,
...
#    header for the exact command and vmdpathfinder/hole_tcl/tests/sph_addend_test.sh for
...
#  bugs at (see vmdpathfinder/hole_tcl/README.md's HOLCAL trace section).
#
...
# byte-verified against the reference-build hole (see vmdpathfinder/hole_tcl/README.md)'s own output, see write_sph).
...
#
#  Byte-identical to the reference-build hole (see vmdpathfinder/hole_tcl/README.md) on the fixture this project's other
#  fidelity claims use (1GRM, cpoint 0 0 0, cvect 0 0 1, sample 0.25, endrad
#  22.0, raseed 1):
#    cd vmdpathfinder/hole_tcl/reference_bin && ./hole < run_sph.inp   # sphpdb ref.sph, same cards (local build - see vmdpathfinder/hole_tcl/README.md)
#    tclsh vmdpathfinder/hole_tcl/hole.tcl -pdb 1GRM.pdb -rad simple.rad -cpoint "0 0 0" \
#        -cvect "0 0 1" -sample 0.25 -endrad 22.0 -seed 1 -sph tcl.sph
#    diff -q ref.sph tcl.sph      # clean
#  See vmdpathfinder/hole_tcl/tests/sph_addend_test.sh for the durable version of this.
#
...
proc hole::main {argv} {
    ...
    # print-precision noise (see vmdpathfinder/hole_tcl/README.md's HOLCAL trace section).
