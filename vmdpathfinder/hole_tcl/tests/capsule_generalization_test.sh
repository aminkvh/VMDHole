...
[ -x "$REFBIN/hole" ] || {
  ...
  echo "        then copy it into vmdpathfinder/hole_tcl/reference_bin/ (local only, never committed - see vmdpathfinder/hole_tcl/README.md)"
  exit 0
}
...
run_case () {
  ...
  if [ -n "$cpoint" ]; then
    ( cd "$TMP" && tclsh "$ROOT/vmdpathfinder/hole_tcl/hole.tcl" -pdb "$bn" -rad s.rad -method capsule \
...
PDB1GRM="$ROOT/vmdpathfinder/1GRM.pdb"
