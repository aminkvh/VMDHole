...
#
# Uses vmdpathfinder/hole_tcl/reference_bin/{hole,sph_process,sos_triangle} (local, never committed) - a build from
...
# be used as a fidelity reference again. See vmdpathfinder/hole_tcl/README.md (Reference binaries section).
#
...
for b in hole sph_process sos_triangle; do
  [ -x "$REFBIN/$b" ] || {
    ...
    echo "        then copy the three binaries into vmdpathfinder/hole_tcl/reference_bin/ (local only - see vmdpathfinder/hole_tcl/README.md)"
    exit 0
  }
done
...
cat > run_sph.tcl <<EOF
source "$ROOT/vmdpathfinder/hole_tcl/sph_process.tcl"
...
cat > run_sos.tcl <<EOF
source "$ROOT/vmdpathfinder/hole_tcl/sph_process.tcl"
source "$ROOT/vmdpathfinder/hole_tcl/sos_triangle.tcl"
