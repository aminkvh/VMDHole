...
shift
# Honour VMDPATHFINDER_HOLE_EXE_DIR like the rest of the suite; $HOME/hole2/exe stays
# the install default. Hardcoding $HOME skipped this comparison on any tree whose
# HOLE lives elsewhere.
REFHOLE=${VMDPATHFINDER_HOLE_EXE_DIR:-$HOME/hole2/exe}/hole
[ -x "$REFHOLE" ] || { echo "SKIP: no reference binary at $REFHOLE"; exit 0; }

TMP=$(mktemp -d); trap 'rm -rf "$TMP"' EXIT
cp "$PDB" "$TMP/in.pdb"
RADSRC="$ROOT/vmdpathfinder/tests/fixtures/simple.rad"
...
tclsh "$ROOT/vmdpathfinder/hole_tcl/hole.tcl" -pdb "$TMP/in.pdb" -rad "$TMP/s.rad" \
