#!/usr/bin/env python3
"""Bundle vmdpathfinder/hole_tcl/ into one self-contained Tcl block for the plugin.
...
`hole::` namespace and executes nothing at source time.
    ...
    python3 vmdpathfinder/hole_tcl/inline_for_plugin.py > /tmp/hole_inlined.tcl

Verified: the bundle produces a .sph byte-identical to the reference-build hole (see vmdpathfinder/hole_tcl/README.md)
...
def expand(match):
    ...
    return (f"# ---- begin {name} (inlined from vmdpathfinder/hole_tcl/) ----\n"
            f"{load(name)}\n# ---- end {name} ----")
