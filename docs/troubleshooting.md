...
| Symptom | Action |
|---|---|
| Profile exists but no mesh | Check `sph_process` and `sos_triangle` paths and their console output. Capsule builds its surface in the plugin and uses neither |
...
| Playback stalls | Pre-build surfaces, increase cache within available memory, or raise the playback triangle count under the `sos_triangle` mesher |
