...
foreach {pm se ce fb want} {
    ...
    set ::VMDHole::state(pore_method) $pm
    set ::VMDHole::state(search_engine) $se
    set ::VMDHole::state(conn_engine) $ce
    chk "mode for $pm/$se/$ce/fallback=$fb" [::VMDHole::_nm_mode_for_run $fb] $want
}

# --- control-file conn stripping for the MC + fast Connolly route
set tmpd [file join [::VMDHole::get_temp_base] "vmdhole_nm_[pid]"]
file delete -force $tmpd; file mkdir $tmpd
array set ::VMDHole::state [list pore_method connolly extra_cards {conn 1.15 0.5; cutsize 0.9} \
    cpoint {0 0 0} cvect {0 0 1} sample 0.5 endrad 8.0 radius_file $RAD ignore {}]
::VMDHole::write_control_file [file join $tmpd a.inp] input_frame.pdb hole_out.sph {0 0 0} {0 0 1}
::VMDHole::write_control_file [file join $tmpd b.inp] input_frame.pdb hole_out.sph {0 0 0} {0 0 1} 1
...
# --- engine + fixtures
set exe [::VMDHole::tool_path nm_search]
if {$exe eq ""} {
    set cand [file join $root native nm nm_search]
    if {[file executable $cand]} { set ::VMDHole::state(nm_search_exec) $cand; set exe $cand }
}
...
if {![file executable $::VMDHole::state(hole_exec)]} { puts "SKIP: nm_engine - no HOLE binary to compare against"; done }
...
proc run_case {label settings} {
    ...
    file mkdir $work
    catch {::VMDHole::clear_results_for_new_settings}
    ...
    array set ::VMDHole::state $settings
    ::VMDHole::validate_inputs
    set t0 [clock milliseconds]
    if {[catch {::VMDHole::run_analysis} err]} {
        chk "$label: run_analysis completes" "error: $err" "no error"
        return {}
    }
    note "$label: run took [expr {[clock milliseconds]-$t0}] ms"
    chk "$label: one result frame" [llength $::VMDHole::result_frames] 1
    if {![llength $::VMDHole::result_frames]} { return {} }
    set f [lindex $::VMDHole::result_frames 0]
    set p [dict get $::VMDHole::results $f profile]
    dict set p run_dir [dict get $::VMDHole::results $f run_dir]
...
lassign [::VMDHole::_hole_coord_identity $sel [file join $d _id.pdb]] _n _idb
::VMDHole::_write_hole_coord_bin $sel [file join $d f.vhb] $_n $_idb
$sel delete
foreach k {pdb vhb} {
    catch {exec $exe {*}[::VMDHole::tool_args nm_search] [file join $d f.$k] $RAD 0 0 0 0 0 1 0.5 8.0 --sph [file join $d $k.sph] --quiet} _
}
...
if {[dict size $A] && [dict size $B]} {
    ...
    set sig [::VMDHole::run_signature $mid all]
