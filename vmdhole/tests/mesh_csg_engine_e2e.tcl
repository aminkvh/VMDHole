...
set exe [::VMDHole::tool_path mesh_csg]
if {$exe eq ""} {
    set cand [file join $root native nm mesh_csg]
    if {[file executable $cand]} { set ::VMDHole::state(mesh_csg_exec) $cand; set exe $cand }
}
if {$exe eq ""} { puts "SKIP: mesh_csg_engine - no mesh_csg binary (sh native/build.sh)"; done }
note "mesh_csg = $exe [::VMDHole::tool_args mesh_csg]"
if {![file readable $PDB] || ![file readable $RAD]} { puts "SKIP: mesh_csg_engine - no 1GRM fixture"; done }
if {![file executable $::VMDHole::state(hole_exec)]} { puts "SKIP: mesh_csg_engine - no HOLE binary"; done }

set mid [mol new $PDB waitfor all]
set work [file join [::VMDHole::get_temp_base] "vmdhole_mc_[pid]"]
...
if {[catch {::VMDHole::run_analysis} err]} { chk "spherical run completes" "error: $err" "no error"; done }
...
# --- gating -----------------------------------------------------------------
chk "csg active for spherical/triangulated/hole_def" [::VMDHole::_csg_active] 1
set ::VMDHole::state(display_mode) wireframe
chk "csg active for wireframe" [::VMDHole::_csg_active] 1
set ::VMDHole::state(display_mode) dots
chk "csg inactive for dots" [::VMDHole::_csg_active] 0
set ::VMDHole::state(display_mode) triangulated
set ::VMDHole::state(surface_color) property
chk "csg serves property colouring, in HOLE's own records" [expr {[::VMDHole::_csg_active] && [::VMDHole::_csg_draw_form]}] 1
set ::VMDHole::state(surface_color) pore_lobes
chk "csg serves lobe colouring the same way" [expr {[::VMDHole::_csg_active] && [::VMDHole::_csg_draw_form]}] 1
set ::VMDHole::state(surface_color) blue
chk "csg active for a plain VMD colour" [::VMDHole::_csg_active] 1
chk "...addressed to the molecule, not draw records" [::VMDHole::_csg_draw_form] 0
set ::VMDHole::state(surface_color) hole_def
set ::VMDHole::state(csg_voxel) 1.0
set ::VMDHole::state(pore_method) connolly
chk "csg serves Connolly too" [::VMDHole::_csg_active] 1
chk "...in HOLE's own records, for the region split" [::VMDHole::_csg_draw_form] 1
chk "...on a uniform grid, no neck to refine" [::VMDHole::_csg_voxel_spec] 1.00
chk "...under a different name" [string match "*_csg1.00_draw.vmd_plot" [::VMDHole::surface_plot_name $work hole_triangulated draw]] 1
set ::VMDHole::state(pore_method) capsule
chk "csg serves capsule" [::VMDHole::_csg_can_mesh] 1
chk "...and hands the mesher the run's axis" [::VMDHole::_csg_mesh_opts] {--axis 0 0 0 0 0 1 12.0}
set ::VMDHole::state(pore_method) circular
set ::VMDHole::state(csg_voxel) 1.0
chk "recipe carries the grid" [string match "*|csg2*1.00/0.50" [::VMDHole::_geom_cache_recipe]] 1
set ::VMDHole::state(csg_voxel) abc
chk "bad grid falls back to the default" [::VMDHole::_csg_voxel 0] 1.40
set ::VMDHole::state(csg_voxel) 1.0

# --- assets ------------------------------------------------------------------
set a0 [::VMDHole::create_plot_asset $rd $sph triangulated $mid $f 0]
...
set a1 [::VMDHole::create_plot_asset $rd $sph triangulated $mid $f 1]
set p1 [dict get $a1 path]
chk "playback and settled are ONE mesh" [expr {$p1 eq $p0}] 1
chk "server channel stays open between frames" [expr {$::VMDHole::_csg_chan ne ""}] 1
chk "surface cached on disk" [::VMDHole::surface_cached_on_disk $rd $sph triangulated] 1
set n0 [llength [::VMDHole::plot_cache_entries $p0]]
...
set gm [::VMDHole::ensure_surface_mol $mid]
chk "mesh is owned by this session" [dict exists $::VMDHole::_csg_owned $p0] 1
catch {graphics $gm delete all}
chk "fast render draws" [::VMDHole::_csg_fast_render $p0 $gm "" Opaque] 1
...
chk "a plain colour override is not fast-pathed" [::VMDHole::_csg_fast_render $p0 $gm blue Opaque] 0
set unowned [file join $work copied.vmd_plot]
file copy -force $p0 $unowned
chk "an unowned plot is never sourced" [::VMDHole::_csg_fast_render $unowned $gm "" Opaque] 0
chk "...but it still parses as data" [expr {[llength [::VMDHole::plot_cache_entries $unowned]] == $n0}] 1
...
chk "one grid spec for playback and settle" [::VMDHole::_csg_voxel_spec] "1.00/0.50"
...
::VMDHole::_csg_server_mesh $marked $mplot 0.5 meshdraw
set far 0
...
set ext [::VMDHole::_csg_sph_extent $sph]
...
set ndot [::VMDHole::_csg_server_mesh $sph $dplot [::VMDHole::_csg_voxel_spec] meshdots]
...
set _sidecar_ok [expr {![catch {::VMDHole::write_hydro3d_atoms_sidecar $mid $f $side kd} _se]}]
if {$_sidecar_ok} {
    set base [::VMDHole::_csg_base_mesh $rd $sph]
    set drawbase [file join $work base_draw.vmd_plot]
    ::VMDHole::_csg_server_mesh $sph $drawbase [::VMDHole::_csg_voxel_spec] meshdraw
    set outc [file join $work col_csg.vmd_plot]
    set ::VMDHole::state(hydro_scheme) kd
    chk "in-process recolour writes a coloured mesh" \
        [::VMDHole::_csg_recolor $drawbase $outc $sph $side 1 -4.5 4.5 1] 1
    ...
    if {[::VMDHole::sos_triangle_has_feature hydro3d] && [::VMDHole::sos_triangle_has_feature hydro3dlining]} {
        set outs [file join $work col_sos.vmd_plot]
        set _sv_m $::VMDHole::state(mesher); set ::VMDHole::state(mesher) sos
        ::VMDHole::run_sos_triangle_3d_recolor $drawbase $outs $sph $side 1 -4.5 4.5 1
        set ::VMDHole::state(mesher) $_sv_m
        set cs [_colseq $outs]
...
chk "the merged binary carries the mesher" [expr {[::VMDHole::tool_args mesh_csg] eq "--mesh" || [file tail $exe] ne "sos_triangle"}] 1

# --- capsule: QC1/QC2 pairs are capsules; escaped slices leave with the axis ---
set CAP [file join $root vmdhole tests fixtures capsule_1GRM.sph]
if {[file readable $CAP]} {
    set cplot [file join $work capsule.vmd_plot]
    catch {exec $exe {*}[::VMDHole::tool_args mesh_csg] $CAP $cplot 1.0/0.5 --draw --axis 0 0 0 0 0 1 8} _
    set cz {}; set nt 0
    ...
    catch {exec $exe {*}[::VMDHole::tool_args mesh_csg] $CAP $cdots 1.0/0.5 --dots --axis 0 0 0 0 0 1 8} _
    set nd 0
    if {![catch {set fh [open $cdots r]}]} { set nd [regexp -all {draw point} [read $fh]]; close $fh }
    chk "...and its dots display" [expr {$nd > 100}] 1
    set _sv [array get ::VMDHole::state {pore_method cpoint cvect endrad}]
    array set ::VMDHole::state {pore_method capsule cpoint {0 0 0} cvect {0 0 1} endrad 8}
    set ext [::VMDHole::_csg_sph_extent $CAP]
    array set ::VMDHole::state $_sv
...
catch {exec $exe {*}[::VMDHole::tool_args mesh_csg] $sph $base 1.0/0.5 --draw} _
set selfw [file join $work selfw.vmd_plot]
catch {exec $exe {*}[::VMDHole::tool_args mesh_csg] $sph $selfw 1.0/0.5 --draw --with $sph $sph} _
...
catch {exec $exe {*}[::VMDHole::tool_args mesh_csg] $sph $winp 1.0/0.5 --draw --with $shifted} _
...
chk "surface_mesh hands the window to the mesher" [expr {[string first {lappend _mopts --with} [info body ::VMDHole::surface_mesh]] >= 0}] 1
...
::VMDHole::_csg_server_mesh $sph $rep1 [::VMDHole::_csg_voxel_spec] meshdraw
::VMDHole::_csg_server_mesh $sph $rep2 [::VMDHole::_csg_voxel_spec] meshdraw
...
set ntri_draw [::VMDHole::_csg_server_mesh $sph $drawmesh [::VMDHole::_csg_voxel_spec] meshdraw]
...
chk "a draw-form mesh is never sourced" [dict exists $::VMDHole::_csg_owned $drawmesh] 0
chk "...but parses as data" [expr {[llength [::VMDHole::plot_cache_entries $drawmesh]] > 100}] 1
...
foreach _p {_tunnel_effective_repr _tunnel_effective_colormode _tunnel_effective_prop} {
    rename ::VMDHole::$_p ::VMDHole::__t_$_p
}
proc ::VMDHole::_tunnel_effective_repr {i} { return $::T_REPR }
proc ::VMDHole::_tunnel_effective_colormode {i} { return $::T_MODE }
proc ::VMDHole::_tunnel_effective_prop {i} { return $::T_PROP }
set ::T_REPR iso; set ::T_MODE auto; set ::T_PROP none
chk "tunnel takes the mesher" [::VMDHole::_tunnel_wants_csg 1] 1
set ::T_REPR wire
chk "wireframe tunnel still takes it" [::VMDHole::_tunnel_wants_csg 1] 1
set ::T_REPR centerline
chk "centerline tunnel needs no mesh" [::VMDHole::_tunnel_wants_csg 1] 0
set ::T_REPR iso; set ::T_MODE property; set ::T_PROP hydropathy
chk "property tunnel takes it too (draw records, no polygon limit)" [::VMDHole::_tunnel_wants_csg 1] 1
set ::T_MODE auto
chk "tunnel mesh name carries the grid" \
    [file tail [::VMDHole::_tunnel_plot $work 3]] tunnel_03_csg1.00_0.50_draw.vmd_plot
set ::VMDHole::state(mesher) sos
chk "tunnel sos name is unchanged" [file tail [::VMDHole::_tunnel_plot $work 3]] tunnel_03.vmd_plot
set ::VMDHole::state(mesher) csg
set ::VMDHole::state(mesher) sos
chk "mesher=sos leaves tunnels alone" [::VMDHole::_tunnel_wants_csg 1] 0
set ::VMDHole::state(mesher) csg
foreach _p {_tunnel_effective_repr _tunnel_effective_colormode _tunnel_effective_prop} {
    rename ::VMDHole::$_p {}
    rename ::VMDHole::__t_$_p ::VMDHole::$_p
}

# --- server robustness --------------------------------------------------------
set bad [file join $work missing.sph]
chk "missing input returns -1" [::VMDHole::_csg_server_mesh $bad [file join $work bad.vmd_plot] 1.0/0.5] -1
chk "server survives a bad request" [expr {$::VMDHole::_csg_chan ne ""}] 1
set again [::VMDHole::_csg_server_mesh $sph [file join $work again.vmd_plot] 1.0/0.5]
chk "server still answers" [expr {$again > 0}] 1
::VMDHole::_csg_server_close
chk "server closed" $::VMDHole::_csg_chan ""
set a2 [::VMDHole::create_plot_asset $rd $sph triangulated $mid $f 0]
chk "cached asset needs no server" [expr {[dict get $a2 path] eq $p0 && $::VMDHole::_csg_chan eq ""}] 1

# --- fallback to sos_triangle -------------------------------------------------
set ::VMDHole::state(mesher) sos
chk "mesher=sos deactivates csg" [::VMDHole::_csg_active] 0
set a3 [::VMDHole::create_plot_asset $rd $sph triangulated $mid $f 0]
chk "sos asset name" [file tail [dict get $a3 path]] hole_triangulated.vmd_plot
chk "sos plot has triangles" [expr {[llength [::VMDHole::plot_cache_entries [dict get $a3 path]]] > 100}] 1
set ::VMDHole::state(mesher) csg
set ::VMDHole::state(mesh_csg_exec) [file join $work no_such_binary]
rename ::VMDHole::_find_exe ::VMDHole::__fe
proc ::VMDHole::_find_exe {p} { return "" }
rename ::VMDHole::auto_execok ::VMDHole::__ae
proc ::VMDHole::auto_execok {p} { return "" }
rename ::VMDHole::sos_triangle_has_feature ::VMDHole::__hf
proc ::VMDHole::sos_triangle_has_feature {f} { return 0 }
chk "missing binary deactivates csg" [::VMDHole::_csg_active] 0
set a4 [::VMDHole::create_plot_asset $rd $sph triangulated $mid $f 0]
chk "missing binary uses the sos mesh" [file tail [dict get $a4 path]] hole_triangulated.vmd_plot
rename ::VMDHole::_find_exe {}; rename ::VMDHole::__fe ::VMDHole::_find_exe
rename ::VMDHole::auto_execok {}; rename ::VMDHole::__ae ::VMDHole::auto_execok
rename ::VMDHole::sos_triangle_has_feature {}; rename ::VMDHole::__hf ::VMDHole::sos_triangle_has_feature
set ::VMDHole::state(mesh_csg_exec) $exe

# --- prebuild pool uses the mesher ------------------------------------------------
file delete $p0
dict set ::VMDHole::results $f asset {}
set ::VMDHole::state(prebuild_surfaces) 1
if {[catch {::VMDHole::prebuild_surfaces_parallel} perr]} { note "prebuild error: $perr" }
chk "prebuild wrote the csg mesh" [file exists $p0] 1
chk "prebuild registered the asset" [expr {[dict exists $::VMDHole::results $f asset path] && [dict get $::VMDHole::results $f asset path] eq $p0}] 1
