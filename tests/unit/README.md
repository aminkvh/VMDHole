...
| suite | scope | needs |
|---|---|---|
| `vmdpathfinder/tests/` | the plugin, end to end | VMD, locally built HOLE binaries, fixtures |
| `vmdpathfinder/hole_tcl/tests/` | the pure-Tcl HOLE engine vs a reference binary | reference `hole` build |
...
4. **A test that genuinely cannot run prints `SKIP:` at column 0 and exits 0** —
   the same contract `vmdpathfinder/tests/run_tests.sh` uses.
...
| `test_skip_message_contract.sh` | Group-level skips written in a form `run_tests.sh`'s `^SKIP:` gate cannot see, which makes a group that checked nothing count as a PASS — and pass the `VMDPATHFINDER_RELEASE=1` gate. |
...
`GROUPS` there and give it a `test_*.sh` wrapper in `vmdpathfinder/tests/`.
