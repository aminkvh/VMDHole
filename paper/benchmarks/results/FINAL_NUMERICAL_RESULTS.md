...
`results/fig_performance.png/.pdf/.eps` regenerated from these CSVs plus
`thread_scaling.csv`; the figure's own agreement check passed (`FIG_RESULT
PASS`, see `fig_performance_provenance.txt`). The end-to-end groups carry a
third bar: the plugin with its own Nelder-Mead search (radius profiles) and
with that search plus the marching-cubes mesher (surfaces). `docs/images/
performance_summary.png` is a copy of the PNG.

A sixth section, "OpenMP threads, jobs=1 (1 -> 8)", isolates each engine's own
thread scaling from the parallel worker-pool effect the other panels measure:
Monte Carlo search and `sos_triangle` show no benefit from OpenMP threads
(1.00x both); the Nelder-Mead search and the marching-cubes mesher do
(1.43x, 1.19x), because their own internal parallelism is what most of their
speedup over the released pipeline comes from - it saturates by 2-4 threads
(`thread_scaling.csv` header machine: AMD Ryzen 7 7700X, 8 physical cores).
